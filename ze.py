...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@https://t.me/alkabtn10"))
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@@f7f7fi"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : الكابتن"
    )
...
async def _(event):
    ...
    21284151 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"21284151 : `{event.sender_id}`")
    else:
